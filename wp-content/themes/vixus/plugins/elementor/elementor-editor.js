	...
	function() {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="vixus_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.vixus_post_editor' );
					if ( link.length > 0 ) {
						vixus_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.vixus_post_editor', function(e) {
					vixus_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('vixus_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function vixus_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
