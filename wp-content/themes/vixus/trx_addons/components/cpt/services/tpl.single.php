	...
	<article id="post-<?php the_ID(); ?>" <?php post_class( 'services_single itemscope' ); trx_addons_seo_snippets('', 'Article'); ?>>
		...
		if (!empty($meta['product']) && (int) $meta['product'] > 0) {
			?><div class="services_page_buttons">
				<a href="<?php the_permalink($meta['product']); ?>" class="sc_button theme_button"><?php esc_html_e('Order now', 'vixus'); ?></a>
			</div><?php
		}
	...
	if ($trx_addons_add_contacts) {
		?><section id="services_page_tab_contacts" class="services_page_section services_page_section_contacts"><?php
			...
			if ( (int) $trx_addons_form_id > 0 ) {
				...
				if ( !function_exists( 'vixus_cpt_services_wpcf7_form_elements' ) ) {
					add_filter('wpcf7_form_elements', 'vixus_cpt_services_wpcf7_form_elements');
					function vixus_cpt_services_wpcf7_form_elements($elements) {
						$elements = str_replace('</textarea>',
												esc_html(sprintf(__("Hi.\nI'd like to order this service '%s'.\nPlease, get in touch with me.", 'vixus'), get_the_title()))
												. '</textarea>',
												$elements
												);
						return $elements;
