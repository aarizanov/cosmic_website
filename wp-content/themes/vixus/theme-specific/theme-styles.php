...
if ( ! function_exists( 'vixus_customizer_get_css' ) ) {

	function vixus_customizer_get_css( $args = array() ) {
		...
		if ( $fonts ) {
			...
			$fonts = vixus_customizer_add_theme_fonts( $fonts );
...
CSS;
			$rez          = apply_filters( 'vixus_filter_get_css', $rez, array( 'fonts' => $fonts ) );
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ( $vars ) {
			...
			// Make theme-specific fonts rules
			$vars = vixus_customizer_add_theme_vars( $vars );
			extract( $vars );
...
CSS;
			$rez          = apply_filters( 'vixus_filter_get_css', $rez, array( 'vars' => $vars ) );
			$css['vars']  = $rez['vars'];
		}
		...
		//--------------------------------------
		if ( false !== $colors ) {
			$schemes = empty( $scheme ) ? array_keys( vixus_get_sorted_schemes() ) : array( $scheme );
			...
			if ( count( $schemes ) > 0 ) {
				...
				foreach ( $schemes as $s ) {
					// Prepare colors
					if ( empty( $scheme ) ) {
						$colors = vixus_get_scheme_colors( $s );
					}
...
.post_item_single .wp-block-archives-dropdown .select_container:before{
...
.post_item_single select:active,
.post_item_single select:focus,
...
.sc_price.sc_price_default .sc_price_columns_wrap>div:nth-child(3n+2) .sc_price_item_link {
	background-color: {$colors['extra_link3']} !important;
}
.sc_price.sc_price_default .sc_price_columns_wrap>div:nth-child(3n+0) .sc_price_item_link {
	background-color: {$colors['extra_hover3']} !important;
}
.sc_price_item_link:hover,
.sc_price.sc_price_default .sc_price_columns_wrap>div:nth-child(3n+2) .sc_price_item_link:hover,
.sc_price.sc_price_default .sc_price_columns_wrap>div:nth-child(3n+0) .sc_price_item_link:hover {
	color: {$colors['bg_color']} !important;
	background-color: {$colors['text_dark']} !important;
}
...
.sc_layouts_menu_nav>li>ul:before {
    border-bottom-color: {$colors['inverse_hover']};
}
...
.vixus_tabs .vixus_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
div.esg-filter-wrapper .esg-filterbutton.selected > span,
.mptt-navigation-tabs li.active a,
.vixus_tabs .vixus_tabs_titles li.ui-state-active a {
	color: {$colors['bg_color']};
	background-color: {$colors['text_dark']};
}
...
.sheme_self.sidebar .vixus_tabs .vixus_tabs_titles li.ui-state-active a {
...
.sticky.post_layout_excerpt .post_title.entry-title a {
    color: {$colors['bg_color']};
}
.sticky.post_layout_excerpt .post_title.entry-title a:hover {
    color: {$colors['text_link']};
}
...
.sidebar .wp-playlist .mejs-container .mejs-controls {
...
.scheme_self.sidebar .wp-calendar-nav-prev a,
.scheme_self.sidebar .wp-calendar-nav-next a,
...
.page_content_wrap .sidebar .widget_calendar td#prev a:before,
.page_content_wrap .sidebar .widget_calendar td#next a:before {
	background-color: {$colors['bg_color']};
}
...
.page_content_wrap .widget_calendar td#prev a:before,
.page_content_wrap .widget_calendar td#next a:before {
	background-color: {$colors['alter_bg_color']};
}
...
.footer_wrap .contacts_description a:hover {
    color: {$colors['text_hover']};
}
...
li.tp-esg-item a:hover {
    color: {$colors['text_link']};
}
li.tp-esg-item.eg-vanburen2-wrapper a {
    color: {$colors['text_hover']};
}
li.tp-esg-item.eg-vanburen2-wrapper a:hover {
    color: {$colors['text_link']};
}
...
.alter-tabs .elementor-tabs .elementor-tab-title a {
    color: {$colors['text']};
}
.alter-tabs .elementor-tabs .elementor-tab-title:hover,
.alter-tabs .elementor-tabs .elementor-tab-title.elementor-active {
    color: {$colors['inverse_hover']};
    background-color: {$colors['inverse_link']};
}
.alter-tabs .elementor-tabs .elementor-tab-title:hover a,
.alter-tabs .elementor-tabs .elementor-tab-title a:hover,
.alter-tabs .elementor-tabs .elementor-tab-title.elementor-active a {
    color: {$colors['inverse_hover']};
}
...
CSS;
					...
					$rez            = apply_filters(
						'vixus_filter_get_css', $rez, array(
							'colors' => $colors,
							'scheme' => $s,
						)
					);
		...
		return apply_filters( 'vixus_filter_prepare_css', $css_str, $remove_spaces );
	}
}
