<div class="front_page_section front_page_section_features<?php
	$vixus_scheme = vixus_get_theme_option( 'front_page_features_scheme' );
	if ( ! vixus_is_inherit( $vixus_scheme ) ) {
		echo ' scheme_' . esc_attr( $vixus_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( vixus_get_theme_option( 'front_page_features_paddings' ) );
?>"
		<?php
		$vixus_css      = '';
		$vixus_bg_image = vixus_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$vixus_anchor_icon = vixus_get_theme_option( 'front_page_features_anchor_icon' );
	$vixus_anchor_text = vixus_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( vixus_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$vixus_css      = '';
			$vixus_bg_mask  = vixus_get_theme_option( 'front_page_features_bg_mask' );
			$vixus_bg_color_type = vixus_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $vixus_bg_color_type ) {
				$vixus_bg_color = vixus_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$vixus_caption = vixus_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $vixus_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $vixus_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $vixus_caption, 'vixus_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$vixus_description = vixus_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $vixus_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $vixus_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $vixus_description ), 'vixus_kses_content' ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
				...
				if ( ! vixus_exists_trx_addons() ) {
					...
					vixus_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
				}
			}
			?>
			</div>
