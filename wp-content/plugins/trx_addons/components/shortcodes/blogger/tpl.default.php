...
if ($show_filters) {
	...
	if (count($args['ids_filters']) > 0) {
		foreach ($args['ids_filters'] as $ids_filter) {
			...
			if ($term) {
				$tabs[$ids_filter] = apply_filters('trx_addons_extended_taxonomy_name', $term->name, $term);
			}
		}
	} else {
		$only_children = $args['cat'] != '' && $args['taxonomy_filters'] == $args['taxonomy'];
		$tabs = $args['taxonomy_filters'] == 'category' && !$only_children
	...
	if (count($tabs) > 0) {
		if (empty($args['filters_active'])) {
			$args['filters_active'] = $args['show_all_filters'] ? 0 : trx_addons_array_get_first($tabs);
		}
